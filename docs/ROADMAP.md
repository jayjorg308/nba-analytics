...
| Jazz surface | 🔨 in progress — planned 2026-09-22 ([the plan doc](plans/jazz-surface.md); ADRs 0081–0085 drafted: tool-class team page rendering from game one, the team shot payload as a fifth contract, feeding from the passing dashboard reconciled to assist credit, the game ledger as box facts + per-game decomposition, human foul annotations as a third class of fact). Increment 1 (the team data spine) built 2026-09-22: team-wide shot + roster pulls, `roster_entry` + the catalog's team dimension (0007), `load_team_season` + hero-less `load_game_pairs`, the shared `team_payload` grammar behind both `derive_team_payload.py` and `export_team_shot_payload.py` (byte parity through the store), the fifth golden pair + Zod contract, `team:sync`, and the season loop's team session (`liveTeams`, dark). Next: increment 2, the `/jazz` page and game ledger, targeted at opening night (week of 2026-10-19) |
