...
v2.5 adds a fourth hero-eligibility gate: every game containing a shot in the hero-season must have a valid canonical live-data play-by-play artifact. A missing whole game is an acquisition failure that can bias assist analysis systematically, so a season with incomplete game-level Case 3 coverage cannot ship as a registered hero once the third payload is required.
