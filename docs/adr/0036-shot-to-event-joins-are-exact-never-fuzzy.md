...
The Case 1 shot and Case 3 play-by-play event join uses exact shot identity: `gameId + gameEventId` on the shot side and, subject to the real-data spike, `gameId + actionNumber` on the live-data side. Player, period, game clock, result, and point value cross-check an identifier match; they may disprove a match but may never manufacture one.
