...
- `npm run hero:scaffold -- <player-slug> <season>` — scaffold a season argument (ADR-0063): hero module (created or seasons-appended), per-season guard skeleton, registry entry; authored fields are `TODO(scaffold)` sentinels the authoring tripwire enforces. Requires all four derived payloads; never overwrites; drafts structure, never judgment.
- `npm run logo:normalize -- public/img/<team>-logo.png` — normalize a team mark to the guarded shared banner-logo canvas before assigning `teamLogoPath`.
...
- **Committed verdict guards**: each season argument's `src/heroes/<slug>.<season>.guard.test.ts` (one guard file per season argument, colocated with its hero's config module, ADR-0022/0063) asserts every directional claim in that season's verdict against its deployed payload's metrics (ADR-0017), and runs the ADR-0063 authoring tripwire (no scaffold sentinel in authored copy, referenced image assets — banner and headshot — exist). If a `hero:sync` or hero swap breaks a claim, rewrite the copy and the guard's claim mapping together — never loosen an assertion.
