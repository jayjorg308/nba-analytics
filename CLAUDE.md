...
- `npm run hero:sync` — copy the latest derived payloads (all four sibling contracts — shot, creation, shot-context, free throw — required per hero, ADR-0030/0032/0053; a partial sync fails) to the committed `public/data/` deployment copy (ADR-0010). No args = every hero in the registry (`src/heroes/registry.ts`); `-- <slug> <season>` syncs one, registered or not.
- `npm run hero:report -- <player-slug> <season>` — print the hero's computed story (gates, ADR-0016 decomposition, per-zone bins/flags, the ADR-0030 creation section, the ADR-0032 assisted-makes section, the ADR-0053 LINE section: season free-throw line with both cuts + the trip taxonomy, and the closing ADR-0059 CLAIM HEADROOM section: verdict-grade gaps vs the house bars, the living-season authoring aid) from the latest derived payloads; `--deployed` reads the committed copies, `--file <path>` (+ optional `--creation-file` / `--context-file` / `--freethrow-file <path>`) any payloads. Every hero swap — and every why-sentence (ADR-0029) — starts by reading this.
...
- **Committed verdict guards**: each hero's `src/heroes/<slug>.guard.test.ts` (colocated with its config module, ADR-0022) asserts every directional claim in that hero's verdict against its deployed payloads' metrics (ADR-0017/0029). Each vocabulary family in a verdict requires its own declared claim kind — creation (Case 2), assist (Case 3, worst-case bounds), free throw (both technical cuts, ADR-0055) — via the shared lexicon + tripwire (`src/heroes/verdictLexicon.ts`); unshipped vocabulary (currently the reserved 'scoring attempt') stays forbidden regardless of claims. If a `hero:sync` or hero swap breaks a claim, rewrite the copy and the guard's claim mapping together — never loosen an assertion.
