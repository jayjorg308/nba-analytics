...
- `npm run hero:report -- <player-slug> <season>` — print the hero's computed story (gates, ADR-0016 decomposition, per-zone bins/flags, and the ADR-0030 creation section: context diet + creation PPS at the product clock grain) from the latest derived payloads; `--deployed` reads the committed copies, `--file <path>` (+ optional `--creation-file <path>`) any payloads. Every hero swap — and every why-sentence (ADR-0029) — starts by reading this.
