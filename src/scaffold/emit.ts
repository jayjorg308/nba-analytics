...
export function emitHeroModule(id: ScaffoldIdentity): string {
  ...
  return [
    ...
    `    headshotPath: ${quoted(`img/${slug}-headshot.png`)},`,
    ...
    `    // imageCredit: 'photograph via the <Team>',`,
    '    // The banner photo described for screen readers. House form: the',
    '    // player as subject in the present tense (rises, hangs, extends,',
    '    // falls away), the action that argues the thesis, then the opponent',
    '    // and the jersey and number. Never "image of" or "photo of" — describe',
    '    // the play, not the file — and a player rises over a DEFENDER, never',
    '    // over a team. Say only what the frame shows (one defender is one).',
