...
**Canonical season**:
The season a hero's `/<slug>` alias renders — the hero's current argument. Recorded explicitly in the hero's registry entry (`canonicalSeason`) and moved only by a flip PR (ADR-0059/0060); the season loop never touches it. The index tile links the canonical alias.
