...
_The repository is `nba-analytics`; the **product is Good Shots** (nbagoodshots.com). That is the name the navbar wordmark carries and the only name product copy should use — the repo name is engineering's, not the reader's. Every naming surface carries the bare wordmark: the navbar, the browser tab (`<title>` and the directory's `document.title`), and the iOS home-screen label. Taglines live in the meta description and the manifest, never in the tab — a tab is a label, not a sentence. Two exceptions carry their own subject, because a reader with several tabs open needs to tell them apart: hero pages replace the title outright (player · season · "shot selection"), and the methodology page prefixes it ("Methodology · Good Shots" — the wordmark stays because, unlike a player's name, a bare "Methodology" identifies nothing; ADR-0071)._
...
**Punctuation style**:
Product copy never uses em dashes — no rendered sentence (component copy, glossary definitions, authored hero copy) may contain "—" as prose punctuation; restructure with a comma, parentheses, or a new sentence instead. The rationale is voice: em-dash-laden prose reads as machine-written. Authored **verdicts** additionally carry no colons or semicolons — the "setup claim: evidence" colon reads as a house tic, and the authoring voice is commas and periods; the remedy is the **period split** (the colon becomes a full stop — "The problem is shot making." is the model), enforced beside the em-dash rule in `src/heroes/heroCopy.test.ts`. Structural copy outside verdicts may still use a colon where a label genuinely introduces. The "—" **no-data placeholder** in data cells and court labels (`EM_DASH` in `src/format.ts`, the assisted-bounds dash) is a glyph, not prose, and stays. Internal docs and code comments are outside the rule.
...
**Usage rate**:
The NBA's official `USG_PCT` for the hero-season: the share of his team's possessions that end with the player's shot, drawn foul, or turnover while he is on the floor. **Sourced, never computed** (ADR-0069): conventional usage math embeds the forbidden 0.44 coefficient, so the product presents the NBA's reported number verbatim — the first number on the page the product cannot verify, fenced by the one fact that is checkable (the source row's FGA must equal the payload's pre-drop season FGA, so the usage figure and the byline's stated frontier describe the same season-to-date record). Descriptive context only, the Matchup stance: role color for reading the verdicts fairly, rendered as a bare byline segment with its definition in the term popover; "usage" is reserved vocabulary, forbidden in verdicts (no claim kind exists). Calibration is definitional only — five players share the floor, so an even share is 20 percent; a computed league-average usage would be a rate average and never ships.
...
**Hero banner**:
The page's poster-scale opening: a black-and-white action image of the hero player carrying the thesis question as the page's `h1` (kicker · question · "↓ the verdict" cue pinned to the banner's bottom edge as the scroll affordance), leading directly into the verdict — ADR-0018's question-first order at poster volume. Its content (image, per-layout focal points, optional `teamLogoPath` team-mark watermark for the wide layout's dark left column) is authored per hero in the hero's config module (`src/heroes/<slug>.ts`), like the verdict; the kicker is season-owned copy carried by each season argument, since it embeds the season string (ADR-0060); its treatment (grayscale filter, wide panel / narrow full-bleed layouts, the display face, the portrait full-viewport landing screen with its graded title zone and glyph halo) is product (ADR-0021, ADR-0020, ADR-0025). The committed image is always a web-sized derivative, never a full-resolution source. Its **alt text** (`imageAlt`) is authored copy in the same sense as the verdict, and takes one house form: the player as subject in the present tense (rises, hangs, extends, falls away), the action that argues the thesis, then the opponent and the jersey and number — never "image of" or "photo of" (describe the play, not the file), and a player rises over a *defender*, never over a team. It says only what the frame shows: one defender is one, and a claim the photo does not support is the same failure as an unguarded verdict claim, minus the test that would have caught it. The banner's reader-facing **credit line** (`imageCredit`, optional) renders in the methodology page's SOURCES section in two forms by what the source states: `photograph by <Name>, <Org>` where a photographer is credited, `photograph via the <Team>` where the team published it with none. A genuinely unknown credit stays absent and renders as "photograph credit pending" — stated, never blank, never guessed (ADR-0071) — while `docs/image-credits.md` keeps the engineering provenance. Team marks use a normalized 1024×1024 transparent canvas with a centered visible mark occupying 58–62% of the longest side; `ingestion/test_team_logo_assets.py` enforces that asset interface so one CSS slot styles every team without per-hero scale overrides, plus a minimum ink-edge ratio so a correctly sized, correctly centered blank (the 2026-08-09 broken-image placeholder that shipped as `was-logo.png` — the record is that asset's entry in `docs/image-credits.md`) fails as content rather than passing as interface; the threshold's derivation lives with the constant in that test, and the remedy for a failure is re-rendering the source, never weakening the bar (ADR-0014's rule, applied to marks).

**Hero index** (a.k.a. **the directory**):
The site root: the first registered hero as a full-width **headshot marquee** (registry order is the directory order — first place is the cover story) over a name-only rail of everyone else on file, each figure the player's standard NBA **headshot** (a transparent-PNG cutout on a controlled dark ground, `headshotPath` in the hero config, the committed `img/<slug>-headshot.png`) with the player's *name* as the copy — the directory answers *who is on file*; the action poster and the thesis stay the hero page's argument (ADR-0065). Each figure links its hero's canonical argument at its `/<slug>` alias (one entry per hero, however many season arguments it carries — ADR-0060), all served by one deployment (ADR-0022). A directory of *arguments*, deliberately not a switcher (ADR-0018): heroes are never view-state under one page, and navigation is plain full-page links (the marquee and rail, the site navbar's wordmark, plus the shared footer's way back from each hero page). Between the marquee and the rail sits the **blurb**: one line stating what the site is, closing with a cue link to the methodology page (ADR-0071) — the directory's whole self-explanation, deliberately not a deck under the wordmark. Everything reads straight off the **hero registry** (`src/heroes/registry.ts`, the single source of hero truth — the index, router, sync script, and per-hero guards all consume it), so registering a hero is what publishes its face here. The marquee's meta eyebrow (team · number · season) is derived from the authored kicker (`indexMetaOf`), never authored twice.

**Site footer**:
The shared sign-off on every page (ROADMAP launch item 4; re-formed under ADR-0071), rendered as a sibling of `<main>` so it is a real `contentinfo` landmark. Three registered zones: **utility links** left (the way back to the directory, the methodology page), the **tagline** dead-center on every page regardless of which utilities render, and the **outward links** right (the social row and the contact address — outward links live here and never in the navbar, ADR-0065). One ring grammar carries both flanks: a 44px bordered icon target in footer gray warming to white. One adaptive rule, so a lone unlabeled glyph never floats: a utility **alone** in its zone spells itself out as a labeled pill (the ring recipe stretched), while utilities in company compress to icon rings. Which utilities render is the page's own fact — the index is the directory so it shows no way back, and the methodology page never links to itself.
...
**Methodology page**:
The site's one self-explanation surface (`/methodology`, ADR-0071): structural copy explaining the model, the honesty rules, the data story, and the verdict stance, re-rendering the dictionary popovers' glossary registry whole as its vocabulary section (one source, so the page can never disagree with a popover) and closing with the SOURCES image credits. Reached from the directory's **blurb** (the one line of self-explanation between the marquee and the rail) and from a Methodology link the shared footer carries on every page; the page itself never links to itself. Never per-hero claims or numbers — no verdict-guard obligations, exactly like the glossary it re-presents. `methodology` is a reserved route: a hero slug may never collide with it (registry coherence guard).
...
**Tracking shortfall**:
The measured size of a hero-season's cross-universe gap: pre-drop season FGA (official) minus the tracking Overall. Zero for most hero-seasons; nonzero only where the season crossed an NBA tracking outage (`season.config.json`'s registry is the live list — ADR-0030, as amended). Persisted in the creation payload, reported in the UI whenever nonzero, and pinned **per outage game** in that registry — one source of truth read by the deployed-pair guard (which asserts the season sum) and by the season loop's coherence check (which needs the per-game grain at mid-season frontiers). Pins are earned only by a characterized outage and move only by PR — the loop never auto-pins or auto-unpins (ADR-0058, as amended) — and a characterization is a bisected localization to specific games (`ingestion/localize_shortfall.py`, ADR-0072), never co-occurrence with a known outage night. A negative shortfall (tracking exceeding the official record) is contradiction, not outage, and hard-fails the derive. Context shares keep the official denominator, so a gapped hero's shares visibly sum short by exactly this reported margin.
...
The raw storage layer is append-only from day one: new snapshots are added, never overwritten. It is mirrored off the dev machine to a private R2 bucket after every season-loop session, copy-only and immutable, so the backup inherits the same rule (ADR-0006, as amended). Derived data recomputes from the latest snapshot for a (player, season). v1 does *not* build snapshot-selection, re-pull scheduling, or diff/merge logic — with one completed-season snapshot, "latest" is trivial. The key carries pull-date so the later live-season demo needs no storage refactor; the machinery that consumes multiple snapshots is deferred until that demo needs it.
...
**Season loop**:
The scheduled daily pipeline that keeps a living season fresh: pull session → the derive phase (since the 2026-08-24 cutover, loads into the record store then the four exports — the file derives remain the explicit fallback engine; ADR-0080) → `hero:sync` → the full repository gate, and on green a data commit pushed for deployment (ADR-0057). Its roster — which hero-seasons are live, and whether each runs dark or publishes — lives in `season.config.json`, changed only by PR. Runs on a dev machine (pulls are local-only; `npm run season:update`), from its own clone kept on `main`: two scheduled windows (06:30 for every session; 23:45 for the team session, so a game can publish the night it ends), a pull of `main` before each, and a **branch guard** that refuses production-store writes and data commits from any other branch. After each session the raw layer is copied off the machine (ADR-0006, as amended). Any failure halts the publish loudly and notifies. For a pre-flip season it runs dark: pull, derive, report, publish nothing. An off-day session ends at the no-change early exit after one discovery pull.
...
**Replay proof**:
The season loop's pre-activation ritual (`ingestion/season_replay.py`): drive the real loop over a calendar of historical frontier dates against a completed season and hold it to oracles computed from the committed payloads — per-day frontier exactness and gate truth, the flip signal firing on exactly the boundary day, and the terminal frame reproducing the committed deployment byte-equal modulo the six provenance fields (pull dates and source paths; ADR-0058, closure). Real data through real endpoints; the calendar is the only fiction. First run 2026-07-23 (Cody 2025-26, every oracle exact — v3's exit); second run 2026-08-24 on the record-store engine against a scratch store (every oracle exact — ADR-0080's cutover gate); rerun before each season's activation.

**Claim headroom**:
The distance between a verdict-grade quantity and the house threshold bars (the verdict ladder's neutral/material/strong/far PPS vocabulary, diet-share ratios, both-cuts free-throw gaps, growth movements — ADR-0061), reported by `hero:report`'s closing CLAIM HEADROOM section. The authoring aid for living-season verdicts (write claims with deliberate margin, since the payload moves daily) — an authoring input only, never a guard input; each hero's guard still declares its own claim mapping with its own rationale, and guards stay exact (ADR-0059).

**Verdict ladder**:
The house pricing of grading words in authored verdicts (`src/heroes/verdictLadder.ts`, ADR-0068) — the same word promises at least the same magnitude on every page. PPS gaps: inside ±0.02 "essentially league average"; past neutral, short of 0.05, hedged ("a little", "a bit"); ≥0.05 a bare comparative ("above", "below", "beat"); ≥0.10 "well above/below"; ≥0.15 "far above/below". Analogs per family: diet leans (0.05 share points bare, 0.10 "far"), free-throw conversion ("well" = 5 FT% points), FTA rate (0.02 floor, "a bit" two-sided up to 0.10, "far" past it). Approximation words ("nearly", "about", "roughly", "close to") are always two-sided bands; "more/fewer than N" stays one-sided. The bars are floors: a guard may declare stricter with rationale, never looser. Consumed by every season argument's guard and printed by CLAIM HEADROOM, so the authoring ruler and the assertions speak one ladder.
